...
\node [mybox] (box){%
    ...
    \small{
        ...
        Total complexity = $O(1/\epsilon^2d)$
    }
    \end{minipage}
};
...
\node [mybox] (box){%
    \begin{minipage}{0.3\textwidth}
    \small{
        Output CC such that: $|CC(G) - C| \leq \epsilon n$, w.p. > $\delta$ \\ 
        ...
        \end{lstlisting}
        Let $Y_j = cost(u_j)$ \\
        $E[Y_j] = \sum \frac{1}{n} cost(u_i) = \frac{1}{n} CC(G)$ \\ 
        $E[\sum Y_j] = s \cdot E[Y_j] = \frac{s}{n} CC(G)$ \\
        Since we output $\frac{n}{s} \sum Y_j$, we get $E[\frac{n}{s} \sum Y_j] = CC(G)$
