...
\begin{multicols*}{3}
        ...
        \node [mybox] (box){%
            \begin{minipage}{0.3\textwidth}
                \small{
                    ...
                    e.g. log($n$)-spanner space with $k$ = log($n$): requires $O(nlogn)$ space. \\
                    e.g. no 3 or 4 length cycle -> $O(n^{3/2})$ edges.
                }
            \end{minipage}
        };
        ...
        \node [mybox] (box){%
            \begin{minipage}{0.3\textwidth}
                \small{
                    ...
                    FM+: Run FM-subroutine $X_1, X_2, ..., X_a$. \\
                    Take average: $Z = \frac{1}{a}\sum\limits_{j=1}^{a}X_j$. \\
                    ...
                    Let $b = 36ln(2/\delta)$, FM++ returns $C \in t(1\pm 4\epsilon)$ w.p. $\geq 1 - \delta$.
                }
            \end{minipage}
        };
        \node[fancytitle, right=10pt] at (box.north west) {Lecture 4 FM};
    \end{tikzpicture}
            ...
            \begin{minipage}{0.3\textwidth}
                \small{
                    \begin{tabular}{|l|c|c|}\hline
                        ...
                        Search BufferTree $\sqrt B$ & $O(log_BN)$                      \\
                        Insert BufferTree $\sqrt B$ & $O((1/\sqrt B) logN)$            \\
        ...
        \node [mybox] (box){%
            \begin{minipage}{0.3\textwidth}
                \small{
                    ...
                    \textbf{BufferTrees:}
                    Amortized cost of split/share/merge is $O(1/B)$, thus $O((1/B) logN)$ per operation. \\
    ...
    \begin{tikzpicture}
        \node [mybox] (box){%
            \begin{minipage}{0.3\textwidth}
                \small{
                    Span of ProcessFrontier (loose) is $O(log^3n)$. \\
                    Work of ProcessFrontier is $O(mlog^2n)$. \\
                    Total work $W = O(mlog^2n)$. \\
                    Total Span $S = O(Dlog^3n)$. \\
                    Totalruntime with $p$ processors and a good scheduler = $O((m/p) log2
                        n + D log^3 n)$
                }
            \end{minipage}
        };
        \node[fancytitle, right=10pt] at (box.north west) {Lecture 11: Parallel BFS};
    \end{tikzpicture}
    ...
    \begin{tikzpicture}
        \node [mybox] (box){%
            \begin{minipage}{0.3\textwidth}
                \small{
                    ...
                    \begin{enumerate}
                        \item \textbf{Divide:} Halve edge list $E$ into $E_1$ and $E_2$. Recursively solve $E_2$, contract $E_1$ with $E_2$, recurse on $E_1$, and merge.
                        \item \textbf{Base case:} $\leq M$ edges, just do in memory.
                        \item \textbf{Contract:} $E_2$ is a depth 1 tree. Edge list is sorted, so we say for $(u, v)$, $u$ is the root. Modify nodes in $E_1$ that are connected to nodes in $E_2$: if $(a, b) \in E_1$ has some $(u, b) \in E_2$, convert $(a, b)$ to $(a, u)$. Do for all different intersecting types. Basically, edges to leaves in $E_2$ can be converted to edges to the root.
                              \begin{itemize}
                                  \item Sort $E_1$ by first components and $E_2$ by second.
                                  \item Scan through $E_1$ and $E_2$ in parallel, marking conversions.
                                  \item Sort $E_1$ by second component and repeat.
                              \end{itemize}
                        \item Recurse on the contracted $E_1$.
                        \item Merge. Do a simple contraction again to make sure we do not have a depth 2 tree.
                    \end{enumerate}
        ...
        \node[fancytitle, right=10pt] at (box.north west) {Lecture 10: CCs in the EMM};
    \end{tikzpicture}
        ...
        \node [mybox] (box){%
            \begin{minipage}{0.3\textwidth}
                \small{
                    ...
                    \begin{itemize}
                        \item Work $W$: how long to complete on 1 processor?
                        \item Span $S$: how long would it take with $p = \infty$?
                        \item Max Parallelism: $W/S$.
                        \item Good scheduler will get you $O(W/p + S)$.
                    \end{itemize}
                    Parallel Sort:
                    \begin{lstlisting}[mathescape]
                        pMergeSort(A, n):
                    ...
                    \end{lstlisting}
                    Work: $W(n)=W(\alpha n) + W((1-\alpha)n) + O(logn)$. \\
                    Span: $S(n)=S(3n/4)+O(logn)=O(log^2n)$.
                }
            \end{minipage}
        };
        \node[fancytitle, right=10pt] at (box.north west) {Lecture 11: PRAM and Fork Join model};
    \end{tikzpicture}
        ...
        \node [mybox] (box){%
            \begin{minipage}{0.3\textwidth}
                \small{
                    ...
                    \begin{tabular}{|l|c|c|}\hline
                        \textbf{Problem} & \textbf{Parallel}                \\ \hline
                        insert           & $W = S = O(logn)$                \\
                        delete           & $W = S = O(logn)$                \\
                        divide           & $W = S = O(logn)$                \\
                        union            & $W = O(n+m), S = O(logn + logm)$ \\
                        set diff/sub     & $W = O(n+m), S = O(logn + logm)$ \\
                        intersection     & $W = O(n+m), S = O(logn + logm)$ \\
                        \hline
                    \end{tabular}
                    Can implement everything with the following operations in (2,4)-tree with:
                    Work = Span = $O(log n + log m)$.
                    \begin{itemize}
                        \item \textbf{Split(T, x)} $\rightarrow$ (T1, T2, x)
                        \item \textbf{Join(T1, T2)} $\rightarrow$ (T)
                        \item \textbf{Root(T)} $\rightarrow$ (x)
                        \item \textbf{Insert(T, x)} $\rightarrow$ (T')
                    \end{itemize}
                    \begin{lstlisting}[mathescape]
                        Union(T1, T2)
                            ...
                            return T
                    \end{lstlisting}
                    $W(n, m) = 2W(n/2, m) + O(logn + logm) = O(nlogm)$. \\
                    $S(n, m) = S(n/2, m) + O(logn + logm) = O(log^2n)$.
                }
            \end{minipage}
        };
        \node[fancytitle, right=10pt] at (box.north west) {Lecture 11: Parallel Sets};
    \end{tikzpicture}
